...
export function CalculatorCharts({ data, result, valid = result.valid, scenarios, active, onActive }: { data: CalculatorData; result: ReturnType<typeof calculate>; valid?: boolean; scenarios: ReturnType<typeof priceScenarios>; active: "structure" | "scenario" | "competitors"; onActive: (value: "structure" | "scenario" | "competitors") => void }) {
  if (!valid || !result.valid) {
    return <div className="surface chart-panel"><div className="surface-title"><h2>Графики расчёта</h2></div><div className="surface-body" role="status"><strong>Исправьте исходные данные</strong><p>{result.issues.find((issue) => issue.blocking)?.message || "Проверьте неверно заполненные числовые поля."}</p><p>Графики и пояснения появятся после корректного расчёта цены.</p></div></div>;
  }
