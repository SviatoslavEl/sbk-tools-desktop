...
export function StaffRegistry() {
  ...
  const filtered = useMemo(() => store.records.filter((record) => {
    ...
    return matches && (!basisFilter || item.basis === basisFilter) && (!statusFilter || item.status === statusFilter) && (!onlyExpiring || expiryInfo(item.documents, expiryDays).warning);
  }), [store.records, search, basisFilter, statusFilter, onlyExpiring, expiryDays]);
  const documentCount = store.records.reduce((sum, record) => sum + record.payload.documents.length, 0);
  const expiringCount = store.records.filter((record) => expiryInfo(record.payload.documents, expiryDays).warning).length;
