...
import { createBackup, importRecordsAtomic, readTextFile, readXlsx, recordHistory, restoreHistoryVersion, writeXlsx, type HistoryEntry, type StoredRecord } from "../../lib/storage";
...
const contractHeaders = ["Номер", "Дата", "Заказчик", "Предмет", "Отрасль", "Вид услуги", "Стандарты", "Состав работ", "Роль в договоре", "Сумма", "Стоимость нашей части", "Начало", "Окончание", "Стадия", "Оплата", "Акты", "Оплачено", "Плановая дата оплаты", "Фактическая дата оплаты", "Важная дата", "Ответственный", "Контакт", "Отзыв", "Раскрытие разрешено", "Примечания"];
const importFields = [
  ...
  ["important", "Важная дата", ["важная дата"]], ["responsible", "Ответственный", ["ответственный"]], ["contact", "Контакт", ["контакт"]], ["review", "Отзыв", ["отзыв"]], ["disclosure", "Раскрытие разрешено", ["раскрытие разрешено", "можно раскрывать"]], ["notes", "Примечания", ["примечания"]],
] as const;
...
function mapContracts(rows: string[][], mapping: Record<ImportField, number>): ContractData[] {
  ...
  return rows.filter((row) => row.some(Boolean)).map((row) => ({ ...emptyContract(), number: value(row, "number"), date: value(row, "date"), customer: value(row, "customer"), subject: value(row, "subject"), industry: value(row, "industry"), serviceType: value(row, "serviceType"), standards: value(row, "standards").split(/[,;]+/).map((entry) => entry.trim()).filter(Boolean), workScope: value(row, "workScope"), contractRole: value(row, "contractRole"), amount: number(value(row, "amount")), ourShareAmount: number(value(row, "ourShare")), startDate: value(row, "start"), endDate: value(row, "end"), stage: contractStages.includes(value(row, "stage") as never) ? value(row, "stage") as ContractData["stage"] : "Подготовка", paymentStatus: paymentStatuses.includes(value(row, "payment") as never) ? value(row, "payment") as ContractData["paymentStatus"] : "Не выставлено", actsStatus: actsStatuses.includes(value(row, "acts") as never) ? value(row, "acts") as ContractData["actsStatus"] : "Не подготовлены", paidAmount: number(value(row, "paid")), paymentPlannedDate: value(row, "paymentPlanned"), paymentActualDate: value(row, "paymentActual"), nextImportantDate: value(row, "important"), responsible: value(row, "responsible"), contact: value(row, "contact"), reviewAvailable: /^(да|yes|true|1)$/i.test(value(row, "review")), disclosureAllowed: /^(да|yes|true|1)$/i.test(value(row, "disclosure")), notes: value(row, "notes") }));
}
...
export function ContractsRegistry() {
  ...
  const exportSelection = async () => {
    const rows = filtered.map(({ payload: item }) => [item.number, item.date, item.customer, item.subject, item.industry, item.serviceType, (item.standards || []).join(", "), item.workScope, item.contractRole, item.amount, item.ourShareAmount, item.startDate, item.endDate, item.stage, item.paymentStatus, item.actsStatus, item.paidAmount, item.paymentPlannedDate, item.paymentActualDate, item.nextImportantDate, item.responsible, item.contact, item.reviewAvailable ? "Да" : "Нет", item.disclosureAllowed ? "Да" : "Нет", item.notes]);
...
function ContractEditor({ record, onSave, onClose }: { record?: StoredRecord<ContractData>; onSave: (item: ContractData, id?: string) => Promise<void>; onClose: () => void }) {
  const [item, setItem] = useState<ContractData>(record ? { ...emptyContract(), ...structuredClone(record.payload), standards: record.payload.standards || [] } : emptyContract());
  ...
  return <aside className="detail-drawer" role="dialog" aria-modal="true" aria-label="Карточка договора"><header><div><h2>{record ? item.number : "Новый договор"}</h2><p>Статусы хранятся независимо</p></div><button className="icon-button" type="button" onClick={onClose}>×</button></header><div className="drawer-body">
    ...
    <h3>Контакты и раскрытие</h3><label>Контакт<input value={item.contact} onChange={(event) => update("contact", event.target.value)} /></label><label className="checkbox-row"><input type="checkbox" checked={item.reviewAvailable} onChange={(event) => update("reviewAvailable", event.target.checked)} /> Есть отзыв / рекомендация</label><label className="checkbox-row"><input type="checkbox" checked={item.disclosureAllowed} onChange={(event) => update("disclosureAllowed", event.target.checked)} /> Разрешено раскрывать договор в заявке</label><label>Примечания<textarea rows={5} value={item.notes} onChange={(event) => update("notes", event.target.value)} /></label>
