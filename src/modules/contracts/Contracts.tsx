...
import { copyAttachment, createBackup, createRegistryArchive, discardStagedAttachments, getWorkspaceInfo, importRecordsAtomic, readDocxTable, readTextFile, readXlsx, recordHistory, restoreHistoryVersion, writeContractReportDocx, writeContractReportPdf, writeXlsx, type ContractReportData, type HistoryEntry, type StoredRecord } from "../../lib/storage";
import { applyImportReviewOverrides, buildImportReviewOverride, importProblemRows, missingImportFields, replaceImportReviewRow, type ImportReviewOverride, type ImportRequiredField } from "../../lib/importReview";
import { actsStatuses, contractStages, emptyContract, emptyContractDocument, paymentStatuses, type ContractData, type ContractDocument } from "./types";
...
const requiredContractImportFields: ImportRequiredField<ContractData>[] = [
  { key: "performingLegalEntity", label: "Юрлицо-исполнитель", missing: (item) => !item.performingLegalEntity.trim() },
  { key: "number", label: "Номер договора", missing: (item) => !item.number.trim() },
  { key: "customer", label: "Заказчик", missing: (item) => !item.customer.trim() },
  { key: "subject", label: "Предмет договора", missing: (item) => !item.subject.trim() },
];
...
export function normalizeContractData(payload: ContractData): ContractData {
  ...
  return {
    ...emptyContract(), ...payload, standards: payload.standards || [], documents: payload.documents || [],
...
export function ContractsRegistry() {
  ...
  const [importReviewedRows, setImportReviewedRows] = useState<Set<number>>(new Set());
  const [importOverrides, setImportOverrides] = useState<Map<number, ImportReviewOverride<ContractData>>>(new Map());
  ...
  const [selectionCriteria, setSelectionCriteria] = useState<ContractSelectionCriteria>({ procurementTitle: "", legalEntity: "", keywords: "", minAmount: 0, maxAmount: 0, amountBasis: "full", industry: "", serviceType: "", contractRole: "", stage: "", endDateFrom: "", endDateTo: "", reviewOnly: false, completedOnly: true, disclosureOnly: true });
  ...
  const industries = [...new Set(store.records.map((record) => record.payload.industry).filter(Boolean))].sort();
  const serviceTypes = [...new Set(store.records.map((record) => record.payload.serviceType).filter(Boolean))].sort();
  const contractRoles = [...new Set(store.records.map((record) => record.payload.contractRole).filter(Boolean))].sort();
  ...
  useEffect(() => {
    ...
    setSelectedContracts((current) => new Set([...current].filter((id) => visibleIds.has(id))));
  }, [matches]);
  ...
  const exportArchive = async (recordIds = filtered.map((record) => record.id)) => {
    if (!recordIds.length) return window.alert("Нет договоров для экспорта.");
    const selected = new Set(recordIds);
    const restricted = store.records.filter((record) => selected.has(record.id) && !record.payload.disclosureAllowed).length;
    if (restricted && !window.confirm(`В архив попадут документы по ${restricted} договорам без разрешения на раскрытие. Продолжить экспорт?`)) return;
    const path = await chooseSavePath("Договоры и все документы", "договоры-с-документами.zip", ["zip"]);
    if (!path) return;
    try {
      const result = await createRegistryArchive("contract-experience", path, recordIds);
  ...
  const reportData = (): ContractReportData => {
    ...
    return { title: selectionCriteria.procurementTitle.trim() ? `Подбор опыта для закупки «${selectionCriteria.procurementTitle.trim()}»` : "Подбор договоров", criteria: [selectionCriteria.legalEntity ? `юрлицо: ${selectionCriteria.legalEntity}` : "", selectionCriteria.keywords ? `ключевые слова: ${selectionCriteria.keywords}` : "", selectionCriteria.industry ? `отрасль: ${selectionCriteria.industry}` : "", selectionCriteria.serviceType ? `вид услуги: ${selectionCriteria.serviceType}` : "", selectionCriteria.contractRole ? `роль: ${selectionCriteria.contractRole}` : "", selectionCriteria.stage ? `стадия: ${selectionCriteria.stage}` : "", selectionCriteria.amountBasis === "ourShare" ? "стоимость нашей части" : "полная стоимость", selectionCriteria.minAmount ? `стоимость от ${money(selectionCriteria.minAmount)}` : "", selectionCriteria.maxAmount ? `стоимость до ${money(selectionCriteria.maxAmount)}` : "", selectionCriteria.endDateFrom ? `окончание с ${date(selectionCriteria.endDateFrom)}` : "", selectionCriteria.endDateTo ? `окончание по ${date(selectionCriteria.endDateTo)}` : "", selectionCriteria.reviewOnly ? "есть отзыв" : "", selectionCriteria.completedOnly ? "только выполненные" : "", selectionCriteria.disclosureOnly ? "разрешённые к раскрытию" : ""].filter(Boolean).join("; "), rows: selected.map(({ record }) => contractReportRow(record.payload)) };
  };
  ...
  const reparseContractImport = (mapping = importSource?.mapping, legalEntity = importLegalEntity) => {
    if (!importSource || !mapping) return [];
    return mapContracts(importSource.rows, mapping, legalEntity);
  };

  const updateContractImportRow = <K extends keyof ContractData>(index: number, key: K, value: ContractData[K]) => {
    setImportRows((current) => {
      ...
      setImportOverrides((existing) => {
        ...
        if (Object.keys(override).length) next.set(index, override); else next.delete(index);
        return next;
      });
  ...
  const contractImportProblems = importRows && importSource ? importProblemRows(importRows, (item, index) => {
    const issues = missingImportFields(item, requiredContractImportFields).map((field) => `Не заполнено: ${field.label}`);
    ...
    if (!importReviewedRows.has(index)) {
      if (rawValue("stage") && !contractStages.includes(rawValue("stage") as never)) issues.push(`Неизвестная стадия «${rawValue("stage")}`);
      if (rawValue("payment") && !paymentStatuses.includes(rawValue("payment") as never)) issues.push(`Неизвестный статус оплаты «${rawValue("payment")}`);
      if (rawValue("acts") && !actsStatuses.includes(rawValue("acts") as never)) issues.push(`Неизвестный статус актов «${rawValue("acts")}`);
    }
    issues.push(...contractChecks(item).filter((check) => check.severity === "error").map((check) => check.message));
    const duplicateInFile = importRows.some((other, otherIndex) => otherIndex < index && other.number.trim().toLowerCase() === item.number.trim().toLowerCase() && other.customer.trim().toLowerCase() === item.customer.trim().toLowerCase() && other.date === item.date && other.subject.trim().toLowerCase() === item.subject.trim().toLowerCase());
    if (duplicateInFile) issues.push("Договор повторяется внутри импортируемого файла");
    if (store.records.some((record) => record.payload.number === item.number && record.payload.customer === item.customer && record.payload.date === item.date && record.payload.subject === item.subject)) issues.push("Такой договор уже есть в реестре");
    return issues;
  }) : [];
  ...
  const commitImport = async () => {
    ...
    for (const [index, item] of importRows.entries()) {
      ...
      const rawValue = (key: ImportField) => importSource.mapping[key] >= 0 ? (raw[importSource.mapping[key]] || "").trim() : "";
      if (!importReviewedRows.has(index)) {
        if (rawValue("stage") && !contractStages.includes(rawValue("stage") as never)) errors.push(`Строка ${index + 2}: неизвестная стадия «${rawValue("stage")}» — откройте «Дополнить» и подтвердите корректное значение`);
        if (rawValue("payment") && !paymentStatuses.includes(rawValue("payment") as never)) errors.push(`Строка ${index + 2}: неизвестный статус оплаты «${rawValue("payment")}» — откройте «Дополнить»`);
        if (rawValue("acts") && !actsStatuses.includes(rawValue("acts") as never)) errors.push(`Строка ${index + 2}: неизвестный статус актов «${rawValue("acts")}» — откройте «Дополнить»`);
      }
  ...
  return <div className="module-stack">
    ...
    <div className="surface table-surface"><div className="table-scroll"><table><thead><tr><th>Номер и дата</th><th>Юрлицо-исполнитель</th><th>Заказчик</th><th>Предмет</th><th>Сумма</th><th>Период</th><th>Стадия</th><th>Оплата</th><th>Акты</th><th>Важная дата</th><th>Ответственный</th><th /></tr></thead><tbody>
      {filtered.map((record) => { const item = record.payload; const checks = contractChecks(item); return <tr key={record.id} onDoubleClick={() => setEditing(record)}><td className="sticky-cell"><button className="link-button" type="button" onClick={() => setEditing(record)}><strong>{item.number}</strong><small>{date(item.date)}</small></button></td><td>{item.performingLegalEntity || <span className="field-error">Не указано</span>}</td><td>{item.customer}</td><td className="wide-cell">{item.subject}{checks.length > 0 && <small className="validation-summary" title={checks.map((check) => check.message).join("\n")}>⚠ {checks.length} замеч.</small>}</td><td>{money(item.amount)}{contractBalance(item).overpayment > 0 && <small className="field-error">Переплата {money(contractBalance(item).overpayment)}</small>}</td><td>{date(item.startDate)} — {date(item.endDate)}</td><td><span className="status neutral">{item.stage}</span></td><td><span className={`status ${item.paymentStatus === "Просрочено" ? "danger" : "neutral"}`}>{item.paymentStatus}</span></td><td><span className="status neutral">{item.actsStatus}</span></td><td>{date(item.nextImportantDate)}</td><td>{item.responsible || "—"}</td><td><div className="row-actions"><button className="secondary small" type="button" aria-label={`Редактировать договор ${item.number}`} onClick={() => setEditing(record)}>Редактировать</button><button className="icon-button danger" type="button" aria-label={`Архивировать ${item.number}`} onClick={() => setArchiving(record)}>×</button></div></td></tr>; })}
    ...
    {selectionOpen && <Dialog title="Подбор договоров под закупку" description="Фильтры используют только фактические сведения реестра. Нерелевантные договоры при заданном запросе исключаются." onClose={() => setSelectionOpen(false)} width="1180px"><div className="dialog-body"><div className="form-grid selection-filters"><label className="wide">Название закупки<input value={selectionCriteria.procurementTitle} onChange={(event) => setSelectionCriteria({ ...selectionCriteria, procurementTitle: event.target.value })} /></label><label className="wide">Ключевые слова и требования<input value={selectionCriteria.keywords} onChange={(event) => setSelectionCriteria({ ...selectionCriteria, keywords: event.target.value })} placeholder="информационная безопасность, аудит, ГОСТ…" /></label><label>Юрлицо-исполнитель<select value={selectionCriteria.legalEntity} onChange={(event) => setSelectionCriteria({ ...selectionCriteria, legalEntity: event.target.value })}><option value="">Любое</option>{legalEntities.map((value) => <option key={value}>{value}</option>)}</select></label><label>Отрасль<select value={selectionCriteria.industry} onChange={(event) => setSelectionCriteria({ ...selectionCriteria, industry: event.target.value })}><option value="">Любая</option>{industries.map((value) => <option key={value}>{value}</option>)}</select></label><label>Вид услуги<select value={selectionCriteria.serviceType} onChange={(event) => setSelectionCriteria({ ...selectionCriteria, serviceType: event.target.value })}><option value="">Любой</option>{serviceTypes.map((value) => <option key={value}>{value}</option>)}</select></label><label>Роль в договоре<select value={selectionCriteria.contractRole} onChange={(event) => setSelectionCriteria({ ...selectionCriteria, contractRole: event.target.value })}><option value="">Любая</option>{contractRoles.map((value) => <option key={value}>{value}</option>)}</select></label><label>Стадия<select value={selectionCriteria.stage} onChange={(event) => setSelectionCriteria({ ...selectionCriteria, stage: event.target.value as ContractSelectionCriteria["stage"] })}><option value="">Любая</option>{contractStages.map((value) => <option key={value}>{value}</option>)}</select></label><label>Считать стоимость<select value={selectionCriteria.amountBasis} onChange={(event) => setSelectionCriteria({ ...selectionCriteria, amountBasis: event.target.value as ContractSelectionCriteria["amountBasis"] })}><option value="full">Полная сумма договора</option><option value="ourShare">Стоимость нашей части</option></select></label><label>Стоимость от<input type="number" min="0" value={selectionCriteria.minAmount} onChange={(event) => setSelectionCriteria({ ...selectionCriteria, minAmount: Number(event.target.value) })} /></label><label>Стоимость до<input type="number" min="0" value={selectionCriteria.maxAmount} onChange={(event) => setSelectionCriteria({ ...selectionCriteria, maxAmount: Number(event.target.value) })} /></label><label>Окончание не раньше<input type="date" value={selectionCriteria.endDateFrom} onChange={(event) => setSelectionCriteria({ ...selectionCriteria, endDateFrom: event.target.value })} /></label><label>Окончание не позже<input type="date" value={selectionCriteria.endDateTo} onChange={(event) => setSelectionCriteria({ ...selectionCriteria, endDateTo: event.target.value })} /></label><label className="checkbox-row"><input type="checkbox" checked={selectionCriteria.completedOnly} onChange={(event) => setSelectionCriteria({ ...selectionCriteria, completedOnly: event.target.checked })} /> Только выполненные</label><label className="checkbox-row"><input type="checkbox" checked={selectionCriteria.reviewOnly} onChange={(event) => setSelectionCriteria({ ...selectionCriteria, reviewOnly: event.target.checked })} /> Только с отзывом</label><label className="checkbox-row"><input type="checkbox" checked={selectionCriteria.disclosureOnly} onChange={(event) => setSelectionCriteria({ ...selectionCriteria, disclosureOnly: event.target.checked })} /> Только разрешённые к раскрытию</label></div><div className="import-summary"><strong>Найдено: {matches.length}; выбрано: {selectedContracts.size}</strong><button className="secondary small" type="button" disabled={!matches.length} onClick={() => setSelectedContracts(new Set(matches.map(({ record }) => record.id)))}>Выбрать все найденные</button></div><div className="table-scroll"><table><thead><tr><th /><th>Балл</th><th>Юрлицо</th><th>Договор</th><th>Заказчик</th><th>Предмет</th><th>Почему подходит</th></tr></thead><tbody>{matches.map(({ record, match }) => <tr key={record.id}><td><input aria-label={`Выбрать ${record.payload.number}`} type="checkbox" checked={selectedContracts.has(record.id)} onChange={(event) => setSelectedContracts((current) => { const next = new Set(current); if (event.target.checked) next.add(record.id); else next.delete(record.id); return next; })} /></td><td><strong>{match.score}</strong></td><td>{record.payload.performingLegalEntity}</td><td>{record.payload.number}</td><td>{record.payload.customer}</td><td>{record.payload.subject}</td><td><small>{match.reasons.join(" · ")}</small></td></tr>)}</tbody></table></div>{!matches.length && <div className="empty-inline">По текущим критериям договоры не найдены. Снимите ограничения или уточните ключевые слова.</div>}</div><footer className="dialog-actions"><button className="secondary" type="button" onClick={() => setSelectionOpen(false)}>Закрыть</button><button className="secondary" type="button" disabled={!selectedContracts.size} onClick={() => void exportArchive([...selectedContracts])}>ZIP + документы</button><button className="secondary" type="button" disabled={!selectedContracts.size} onClick={() => void exportReport("xlsx")}>Excel</button><button className="secondary" type="button" disabled={!selectedContracts.size} onClick={() => void exportReport("docx")}>Word</button><button className="primary" type="button" disabled={!selectedContracts.size} onClick={() => void exportReport("pdf")}>PDF</button></footer></Dialog>}
    {importRows && importSource && <Dialog title="Проверка импорта договоров" description="Показываем только строки, которые нужно дополнить. Готовые строки не перегружают список; пакет сохранится одной транзакцией." onClose={() => { setImportRows(null); setImportSource(null); setImportEditingIndex(null); setImportOverrides(new Map()); }} width="880px"><div className="dialog-body">
      <label>Быстрый шаблон юрлица для пустых строк — необязательно<input value={importLegalEntity} onChange={(event) => { const legalEntity = event.target.value; setImportLegalEntity(legalEntity); setImportRows(applyImportReviewOverrides(mapContracts(importSource.rows, importSource.mapping, legalEntity), importOverrides)); setImportReport(null); }} placeholder="Например, ООО «СБК»" /><small>Индивидуально указанное юрлицо имеет приоритет и не будет перезаписано.</small></label>
      <details><summary>Настроить колонки</summary><div className="mapping-grid">{importFields.map(([key, label]) => <label key={key}>{label}<select value={importSource.mapping[key]} onChange={(event) => { const mapping = { ...importSource.mapping, [key]: Number(event.target.value) }; setImportSource({ ...importSource, mapping }); setImportRows(applyImportReviewOverrides(mapContracts(importSource.rows, mapping, importLegalEntity), importOverrides)); setImportReport(null); }}><option value={-1}>Не импортировать</option>{importSource.headers.map((header, index) => <option key={`${header}-${index}`} value={index}>{header || `Колонка ${index + 1}`}</option>)}</select></label>)}</div></details>
      <div className="import-summary"><strong>Всего: {importRows.length} · готовы: {importRows.length - contractImportProblems.length} · требуют дополнения: {contractImportProblems.length}</strong><span>{contractImportProblems.length ? "Заполните отмеченные поля текущей строки — затем автоматически появится следующая проблемная строка." : "Все строки проверены и готовы к атомарному импорту."}</span></div>
      {contractImportProblems[0] ? (() => { const problem = contractImportProblems[0]; const item = problem.item; const missing = new Set(missingImportFields(item, requiredContractImportFields).map((field) => field.key)); return <section className="document-card"><div className="document-card-header"><strong>Строка {problem.index + 2}: {item.number || item.customer || "договор без номера"}</strong><span className="status warning">Осталось {contractImportProblems.length}</span></div><div className="notice warning">{problem.issues.map((issue) => <span key={issue}>{issue}</span>)}</div><div className="form-grid">{missing.has("performingLegalEntity") && <label className="wide">Юрлицо-исполнитель *<input autoFocus value={item.performingLegalEntity} onChange={(event) => updateContractImportRow(problem.index, "performingLegalEntity", event.target.value)} /></label>}{missing.has("number") && <label>Номер договора *<input value={item.number} onChange={(event) => updateContractImportRow(problem.index, "number", event.target.value)} /></label>}{missing.has("customer") && <label className="wide">Заказчик *<input value={item.customer} onChange={(event) => updateContractImportRow(problem.index, "customer", event.target.value)} /></label>}{missing.has("subject") && <label className="wide">Предмет договора *<textarea rows={3} value={item.subject} onChange={(event) => updateContractImportRow(problem.index, "subject", event.target.value)} /></label>}</div><button className="secondary" type="button" onClick={() => setImportEditingIndex(problem.index)}>Открыть карточку для исправления остальных замечаний</button></section>; })() : <div className="notice success"><strong>Проверка завершена</strong><span>Проблемных строк нет. Можно импортировать весь пакет.</span></div>}
      {importReport && <pre className="import-report">{importReport}</pre>}
    </div><footer className="dialog-actions"><button className="secondary" type="button" onClick={() => { setImportRows(null); setImportSource(null); setImportEditingIndex(null); setImportOverrides(new Map()); }}>Отмена</button><button className="primary" disabled={contractImportProblems.length > 0} type="button" onClick={() => void commitImport()}>Импортировать {importRows.length} записей</button></footer></Dialog>}
    {importRows && importEditingIndex !== null && importRows[importEditingIndex] && <ContractEditor initialValue={importRows[importEditingIndex]} onClose={() => setImportEditingIndex(null)} onSave={async (item) => { const reviewedIndex = importEditingIndex; const baseline = reparseContractImport()[reviewedIndex] || importRows[reviewedIndex]; const override = buildImportReviewOverride(baseline, item); setImportRows((current) => current ? replaceImportReviewRow(current, reviewedIndex, item) : null); setImportReviewedRows((current) => new Set(current).add(reviewedIndex)); setImportOverrides((current) => { const next = new Map(current); if (Object.keys(override).length) next.set(reviewedIndex, override); else next.delete(reviewedIndex); return next; }); setImportReport(null); setImportEditingIndex(null); }} />}
  </div>;
}

function ContractEditor({ record, initialValue, onSave, onClose }: { record?: StoredRecord<ContractData>; initialValue?: ContractData; onSave: (item: ContractData, id?: string) => Promise<void>; onClose: () => void }) {
  const [recordId] = useState(() => record?.id || crypto.randomUUID());
  const [item, setItem] = useState<ContractData>(record ? normalizeContractData(structuredClone(record.payload)) : initialValue ? normalizeContractData(structuredClone(initialValue)) : emptyContract());
  const [error, setError] = useState("");
  const [history, setHistory] = useState<HistoryEntry[]>([]);
  useEffect(() => { if (record) void recordHistory("contract-experience", record.id).then(setHistory); }, [record]);
  useEffect(() => () => { void discardStagedAttachments("contract-experience", recordId); }, [recordId]);
  const update = <K extends keyof ContractData>(key: K, value: ContractData[K]) => setItem((current) => ({ ...current, [key]: value }));
  const close = async () => { await discardStagedAttachments("contract-experience", recordId).catch(() => undefined); onClose(); };
  const submit = async () => {
  ...
  return <aside className="detail-drawer" role="dialog" aria-modal="true" aria-label="Карточка договора"><header><div><h2>{record || initialValue ? item.number || "Дополнение импортируемого договора" : "Новый договор"}</h2><p>{initialValue ? "Изменения применяются только к этой строке импорта" : "Статусы хранятся независимо"}</p></div><button className="icon-button" type="button" onClick={() => void close()}>×</button></header><div className="drawer-body">
    ...
    {!initialValue && <ContractDocumentsEditor recordId={recordId} documents={item.documents} onChange={(documents) => update("documents", documents)} />}
  ...
  </div><footer><button className="secondary" type="button" onClick={() => void close()}>Отмена</button><button className="primary" type="button" onClick={() => void submit()}>Сохранить договор</button></footer></aside>;
}

function ContractDocumentsEditor({ recordId, documents, onChange }: { recordId: string; documents: ContractDocument[]; onChange: (items: ContractDocument[]) => void }) {
  const update = (id: string, patch: Partial<ContractDocument>) => onChange(documents.map((item) => item.id === id ? { ...item, ...patch } : item));
  const addFile = async (document: ContractDocument) => {
    const path = await chooseOpenPath("Выберите документ договора", ["pdf", "png", "jpg", "jpeg", "doc", "docx", "xlsx"]);
    if (!path) return;
    const attachment = await copyAttachment(path, "contract-experience", recordId);
    update(document.id, { relativePath: attachment.relativePath, fileName: attachment.fileName, sizeBytes: attachment.sizeBytes, sha256: attachment.sha256, mimeType: attachment.mimeType });
  };
  const openFile = async (document: ContractDocument) => {
    ...
    await openPath(`${workspace.root}${separator}${document.relativePath.replace(/\//g, separator)}`);
  };
  return <><div className="inline-heading"><div><h3>Документы договора</h3><p>Договор, акты, отзывы, сертификаты и другие подтверждения попадут в общий ZIP-архив.</p></div><button className="primary" type="button" onClick={() => onChange([...documents, emptyContractDocument()])}>Добавить документ</button></div>{documents.length === 0 && <div className="empty-inline">Документы пока не прикреплены.</div>}{documents.map((document) => <section className="document-card" key={document.id}><div className="document-card-header"><select aria-label="Тип документа договора" value={document.type} onChange={(event) => update(document.id, { type: event.target.value as ContractDocument["type"] })}><option>Договор</option><option>Акт</option><option>Отзыв</option><option>Сертификат</option><option>Иное</option></select><button className="icon-button danger" type="button" aria-label={`Удалить ${document.name || document.type}`} onClick={() => { if (window.confirm(`Удалить «${document.name || document.type}» из карточки?`)) onChange(documents.filter((item) => item.id !== document.id)); }}>×</button></div><div className="form-grid"><label className="wide">Название<input value={document.name} onChange={(event) => update(document.id, { name: event.target.value })} /></label><label className="wide">Комментарий<textarea rows={2} value={document.comment} onChange={(event) => update(document.id, { comment: event.target.value })} /></label></div><div className="document-file">{document.fileName ? <span title={document.sha256}>▧ {document.fileName}{document.sizeBytes ? ` · ${(document.sizeBytes / 1024 / 1024).toFixed(1)} МБ` : ""}</span> : <span className="muted">Файл не прикреплён</span>}<div className="button-row">{document.relativePath && <button className="secondary small" type="button" onClick={() => void openFile(document)}>Открыть</button>}<button className="secondary small" type="button" onClick={() => void addFile(document)}>{document.fileName ? "Заменить файл" : "Добавить файл"}</button></div></div></section>)}</>;
}
