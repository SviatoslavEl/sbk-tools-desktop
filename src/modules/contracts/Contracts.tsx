...
import { applyImportReviewOverrides, replaceImportReviewRow } from "../../lib/importReview";
...
export function ContractsRegistry() {
  ...
  const [importOverrides, setImportOverrides] = useState<Map<number, ContractData>>(new Map());
  ...
  return <div className="module-stack">
    ...
    {importRows && importSource && <Dialog title="Предпросмотр импорта" description="Сопоставьте колонки и дополните каждую карточку отдельно. Пакет проверяется и сохраняется только целиком." onClose={() => { setImportRows(null); setImportSource(null); setImportEditingIndex(null); setImportOverrides(new Map()); }} width="1080px"><div className="dialog-body"><label>Общее юрлицо-исполнитель для строк без этой колонки<input value={importLegalEntity} onChange={(event) => { const legalEntity = event.target.value; setImportLegalEntity(legalEntity); setImportRows(applyImportReviewOverrides(mapContracts(importSource.rows, importSource.mapping, legalEntity), importOverrides)); setImportReport(null); }} placeholder="Например, ООО «СБК»" /></label><div className="mapping-grid">{importFields.map(([key, label]) => <label key={key}>{label}<select value={importSource.mapping[key]} onChange={(event) => { const mapping = { ...importSource.mapping, [key]: Number(event.target.value) }; setImportSource({ ...importSource, mapping }); setImportRows(applyImportReviewOverrides(mapContracts(importSource.rows, mapping, importLegalEntity), importOverrides)); setImportReport(null); }}><option value={-1}>Не импортировать</option>{importSource.headers.map((header, index) => <option key={`${header}-${index}`} value={index}>{header || `Колонка ${index + 1}`}</option>)}</select></label>)}</div><div className="import-summary"><strong>Строк к импорту: {importRows.length}; дополнено вручную: {importOverrides.size}</strong><span>Ручные дополнения сохраняются при изменении сопоставления колонок. При любой ошибке реестр не изменяется.</span></div><div className="table-scroll import-review-table"><table><thead><tr><th>#</th><th>Юрлицо</th><th>Номер</th><th>Дата</th><th>Заказчик</th><th>Предмет</th><th>Сумма</th><th /></tr></thead><tbody>{importRows.map((item, index) => <tr key={`${item.number}-${index}`}><td>{index + 1}{importOverrides.has(index) ? " ✓" : ""}</td><td>{item.performingLegalEntity || <span className="field-error">нет</span>}</td><td>{item.number || <span className="field-error">нет</span>}</td><td>{item.date}</td><td>{item.customer || <span className="field-error">нет</span>}</td><td>{item.subject || <span className="field-error">нет</span>}</td><td>{money(item.amount)}</td><td><button className="secondary small" type="button" onClick={() => setImportEditingIndex(index)}>Дополнить</button></td></tr>)}</tbody></table></div>{importReport && <pre className="import-report">{importReport}</pre>}</div><footer className="dialog-actions"><button className="secondary" type="button" onClick={() => { setImportRows(null); setImportSource(null); setImportEditingIndex(null); setImportOverrides(new Map()); }}>Отмена</button><button className="primary" type="button" onClick={() => void commitImport()}>Проверить и импортировать пакет</button></footer></Dialog>}
    {importRows && importEditingIndex !== null && importRows[importEditingIndex] && <ContractEditor initialValue={importRows[importEditingIndex]} onClose={() => setImportEditingIndex(null)} onSave={async (item) => { const reviewedIndex = importEditingIndex; setImportRows((current) => current ? replaceImportReviewRow(current, reviewedIndex, item) : null); setImportReviewedRows((current) => new Set(current).add(reviewedIndex)); setImportOverrides((current) => new Map(current).set(reviewedIndex, item)); setImportReport(null); setImportEditingIndex(null); }} />}
  </div>;
}
