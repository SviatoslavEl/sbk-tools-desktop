...
import { readTextFile, readXlsx, recordHistory, writeXlsx, type HistoryEntry, type StoredRecord } from "../../lib/storage";
...
export function ContractsRegistry() {
  ...
  const commitImport = async () => {
    ...
    for (const [index, item] of importRows.entries()) {
      ...
      if (duplicate) {
        errors.push(`Строка ${index + 2}: возможный дубль ${item.number}`);
        continue;
      }
      try {
        await store.save(`${item.number} — ${item.customer}`, item);
        success += 1;
      } catch (reason) {
        errors.push(`Строка ${index + 2}: ${String(reason)}`);
      }
    }
  ...
  return <div className="module-stack">
    ...
    <div className="surface table-surface"><div className="table-scroll"><table><thead><tr><th>Номер и дата</th><th>Заказчик</th><th>Предмет</th><th>Сумма</th><th>Период</th><th>Стадия</th><th>Оплата</th><th>Акты</th><th>Важная дата</th><th>Ответственный</th><th /></tr></thead><tbody>
      {filtered.map((record) => { const item = record.payload; return <tr key={record.id} onDoubleClick={() => setEditing(record)}><td className="sticky-cell"><button className="link-button" type="button" onClick={() => setEditing(record)}><strong>{item.number}</strong><small>{date(item.date)}</small></button></td><td>{item.customer}</td><td className="wide-cell">{item.subject}</td><td>{money(item.amount)}</td><td>{date(item.startDate)} — {date(item.endDate)}</td><td><span className="status neutral">{item.stage}</span></td><td><span className={`status ${item.paymentStatus === "Просрочено" ? "danger" : "neutral"}`}>{item.paymentStatus}</span></td><td><span className="status neutral">{item.actsStatus}</span></td><td>{date(item.nextImportantDate)}</td><td>{item.responsible || "—"}</td><td><button className="icon-button danger" type="button" aria-label={`Архивировать ${item.number}`} onClick={() => setArchiving(record)}>×</button></td></tr>; })}
    ...
    {importRows && importSource && <Dialog title="Предпросмотр импорта" description="Сопоставьте колонки файла с полями реестра." onClose={() => { setImportRows(null); setImportSource(null); }} width="980px"><div className="dialog-body"><div className="mapping-grid">{importFields.map(([key, label]) => <label key={key}>{label}<select value={importSource.mapping[key]} onChange={(event) => { const mapping = { ...importSource.mapping, [key]: Number(event.target.value) }; setImportSource({ ...importSource, mapping }); setImportRows(mapContracts(importSource.rows, mapping)); }}><option value={-1}>Не импортировать</option>{importSource.headers.map((header, index) => <option key={`${header}-${index}`} value={index}>{header || `Колонка ${index + 1}`}</option>)}</select></label>)}</div><div className="import-summary"><strong>Строк к импорту: {importRows.length}</strong><span>Показаны первые 10. Ошибка одной строки не отменит остальные.</span></div><div className="table-scroll"><table><thead><tr><th>Номер</th><th>Дата</th><th>Заказчик</th><th>Предмет</th><th>Сумма</th><th>Стадия</th></tr></thead><tbody>{importRows.slice(0, 10).map((item, index) => <tr key={`${item.number}-${index}`}><td>{item.number || <span className="field-error">нет</span>}</td><td>{item.date}</td><td>{item.customer || <span className="field-error">нет</span>}</td><td>{item.subject || <span className="field-error">нет</span>}</td><td>{money(item.amount)}</td><td>{item.stage}</td></tr>)}</tbody></table></div>{importReport && <pre className="import-report">{importReport}</pre>}</div><footer className="dialog-actions"><button className="secondary" type="button" onClick={() => { setImportRows(null); setImportSource(null); }}>Отмена</button><button className="primary" type="button" onClick={() => void commitImport()}>Импортировать корректные строки</button></footer></Dialog>}
  </div>;
}
