...
interface ScannerRecord { kind: "facsimile-template" | "processing-journal"; relativePath?: string; fileName?: string; inputType?: string; pageCount?: number; preset?: string; ocr?: boolean; status?: string; durationMs?: number; }
...
export function Scanner() {
  ...
  const makePreview = async (path = inputPath, selectedPreset = preset, selectedPage = pageIndex, includeFacsimile = true) => {
    ...
    try {
      const response = await invoke<{ outputPath: string; originalPath?: string; pageCount: number; warnings?: string[]; estimatedOutputBytes?: number; pageSizePoints?: [number, number] }>("scanner_run", {
        jobId, operation: "preview", config: { protocolVersion: 2, inputPath: path, preset: selectedPreset, pageIndex: selectedPage, seed: 42, settings: { dpi, jpeg_quality: quality }, facsimiles: [...savedFacsimiles, ...(previewFacsimile ? [previewFacsimile] : [])], redactions: redactions.map((entry) => ({ pages: [entry.page], x: entry.x, y: entry.y, width: entry.width, height: entry.height, color: entry.color })) },
      });
  ...
  const chooseDocument = async () => {
    ...
    if (!path) return;
    setInputPath(path); setDocumentName(path.split(/[\\/]/).pop() || path); setPageIndex(0); setPageOrder([]); setResultPath(""); setWarnings([]); setFacsimile(null); setSavedFacsimiles([]); setRedactions([]); setOcrConfidence(null); setFacsimilePreviewCommitted(false);
  ...
  const processBatch = async () => {
    ...
    for (const [index, path] of batchPaths.entries()) {
      ...
      try { await invoke("scanner_run", { jobId, operation: "process", config: { protocolVersion: 2, inputPath: path, outputPath, preset, ocrEnabled, ocrLanguages, seed: 42, settings: { dpi, jpeg_quality: quality }, pageOrder: [], redactions: [] } }); }
  ...
  const processDocument = async () => {
    ...
    try {
      const response = await invoke<{ outputPath: string; warnings?: string[]; ocrConfidence?: number | null }>("scanner_run", { jobId, operation: "process", config: { protocolVersion: 2, inputPath, outputPath, preset, ocrEnabled, ocrLanguages, seed: 42, settings: { dpi, jpeg_quality: quality }, facsimiles: [...savedFacsimiles, ...(workerFacsimile ? [workerFacsimile] : [])], pageOrder, redactions: redactions.map((entry) => ({ pages: [entry.page], x: entry.x, y: entry.y, width: entry.width, height: entry.height, color: entry.color })) } });
      setResultPath(response.outputPath || outputPath); setWarnings(response.warnings || []); setOcrConfidence(response.ocrConfidence ?? null); setProgress({ stage: "Готово", currentPage: pageOrder.length, totalPages: pageOrder.length, percent: 100 });
      await templates.save(`Обработка ${new Date().toLocaleString("ru-RU")}`, { kind: "processing-journal", inputType: inputPath.toLowerCase().endsWith(".docx") ? "DOCX" : "PDF", pageCount: pageOrder.length, preset, ocr: ocrEnabled, status: "completed", durationMs: Date.now() - startedAt });
  ...
  return <div className="scanner-layout">
    ...
    <section className={`surface preview-panel ${fullscreen ? "fullscreen-preview" : ""}`}><div className="surface-title"><h2>Предпросмотр</h2><div className="button-row">{originalUrl && <button className="secondary small" type="button" onClick={() => setShowOriginal((value) => !value)}>{showOriginal ? "Показать обработку" : "Показать оригинал"}</button>}<button className="secondary small" type="button" onClick={() => setFullscreen((value) => !value)}>{fullscreen ? "Закрыть полный экран" : "На весь экран"}</button><span>{pageCount ? `Страница ${pageIndex + 1} из ${pageCount}` : "Файл не выбран"} · {preset}</span></div></div>
      {inputPath && <div className="page-editor"><span>Итоговый порядок ({pageOrder.length}):</span><div>{pageOrder.map((sourceIndex) => <button key={sourceIndex} className={sourceIndex === pageIndex ? "active" : ""} type="button" onClick={() => { setPageIndex(sourceIndex); void makePreview(inputPath, preset, sourceIndex); }}>{sourceIndex + 1}</button>)}</div><button className="secondary small" type="button" onClick={() => moveCurrentPage(-1)}>← Раньше</button><button className="secondary small" type="button" onClick={() => moveCurrentPage(1)}>Позже →</button><button className="secondary small danger" type="button" onClick={deleteCurrentPage}>Удалить страницу</button><button className="link-button" type="button" onClick={() => setPageOrder(Array.from({ length: pageCount }, (_, index) => index))}>Сбросить</button></div>}
