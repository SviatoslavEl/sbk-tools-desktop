...
import {
  ...
  previewCacheKey,
  type FacsimilePlacementPayload,
...
export function Scanner() {
  ...
  const [savedFacsimiles, setSavedFacsimiles] = useState<FacsimilePlacementPayload[]>([]);
  ...
  const workerFacsimile = facsimile && facsimileSelection.selection
    ? buildFacsimilePlacement(facsimile, facsimileSelection.selection)
    : null;
  ...
  const chooseDocument = async () => {
    ...
    setInputPath(path); setDocumentName(path.split(/[\\/]/).pop() || path); setPageIndex(0); setPageOrder([]); setPageRotations({}); setResultPath(""); setWarnings([]); setFacsimile(null); setSavedFacsimiles([]); setRedactions([]); setOcrConfidence(null); setOcrText(""); setLowConfidenceWords([]); setPreviewZoom(1);
  ...
  const nudge = (event: KeyboardEvent<HTMLDivElement>) => {
    ...
    setFacsimile({
      ...facsimile,
      x: Math.max(0, Math.min(1 - facsimile.width, facsimile.x + delta[0])),
      y: Math.max(0, Math.min(0.95, facsimile.y + delta[1])),
    });
  };
  ...
  const visibleSavedFacsimiles = savedFacsimiles.filter((placement) => facsimileAppliesTo(placement, pageIndex));
      ...
      {!inputPath ? <div className="drop-empty" onClick={() => void chooseDocument()}><span>▧</span><h2>Выберите документ</h2><p>PDF или DOCX до 200 МБ. Исходный файл не изменяется.</p><button className="primary" type="button">Выбрать файл</button></div> : <div className="preview-workspace">
        ...
        <div className="document-stage">
          <div className="document-page real-preview" style={{ aspectRatio: String(previewAspect), width: `${72 * previewZoom}%`, maxWidth: `${720 * previewZoom}px` }} onPointerMove={moveFacsimile} onPointerUp={() => { dragging.current = false; }}>
            ...
            {!showOriginal && visibleSavedFacsimiles.map((placement, index) => <div className="facsimile real saved" key={`${placement.imagePath}-${index}`} style={facsimileOverlayStyle(placement)}><img src={placement.imageUrl} alt="Зафиксированное факсимиле" style={{ opacity: placement.opacity, mixBlendMode: placement.removeLightBackground ? "multiply" : "normal" }} /></div>)}
            {!showOriginal && visibleOnCurrentPage && workerFacsimile && <div className="facsimile real" role="button" tabIndex={0} aria-label="Факсимиле: перемещайте стрелками" style={facsimileOverlayStyle(workerFacsimile)} onKeyDown={nudge} onPointerDown={(event) => { dragging.current = true; event.currentTarget.setPointerCapture(event.pointerId); }}><img src={facsimile.imageUrl} alt="Факсимиле" style={{ opacity: facsimile.opacity, mixBlendMode: facsimile.removeLightBackground ? "multiply" : "normal" }} /></div>}
          </div>
        </div>
      </div>}
